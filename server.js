...
(async () => {
  ...
  router = await worker.createRouter({
    mediaCodecs: [
      // описываем, какие медиаформаты поддерживает наш router.
      {
...
async function createTransport(socket) {
  ...
  transport.on("dtlsstatechange", (dtlsState) => {
    if (dtlsState === "closed") {
      // если состояние изменилось на "закрыто", закрываем транспорт.
      transport.close();
    }
  });
...
const PORT = process.env.PORT || 3000; // Используйте PORT из окружения Render
server.listen(PORT, () => {
